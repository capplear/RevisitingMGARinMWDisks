...
import gc
dire = r'/home/chenao/for_draft/other_pot/'
def linear_func (p,x):
    return p[0]*x + p[1]
...
def thin_and_thick_grad(i,data,data_name='W23', a=-0.3, b=0.12, k=-0.14,rg_name='rg',feh_name='FEH_APOGEE',age_name='age',erg_name='erg',afe_name='AFE_APOGEE',zmax_name='zmax',efeh_name='err_feh_apogee'):
    ...
    print(thin_meta_grad)
    np.save(dire + f"thin_meta_grad{i}.npy",thin_meta_grad)
    np.save(dire + f"thin_err{i}.npy",thin_err)
    np.save(dire + f"thick_meta_grad{i}.npy",thick_meta_grad)
    np.save(dire + f"thick_err{i}.npy",thick_err)
    return 0
print('func def. done!')
...
conditions = {'pots': [MWPotential2014,McMillan17,Irrgang13I],
              'ros': [7.0,8.27,9.5]*1000,
              'vos':[260, 236, 200]}
for i in [0]:
    if i<3:
        pot_ = conditions['pots'][i]
        ro = 8.27*1000
        vo = 236
    elif 3<=i<6:
        pot_ = MWPotential2014
        ro = conditions['ros'][i-3]
        vo = 236
    else:
        pot_ = MWPotential2014
        ro = 8.27*1000
        vo = conditions['vos'][i-6]
    ...
    np.save(dire + f'rg_{i}.npy',rg)
    np.save(dire + f'erg_{i}.npy',erg)
    for j in range(8):
        ...
        ecc = o.e()
        del o
        np.save(dire + f'zmax_40k_{i}_{j}.npy',zmax)
        np.save(dire + f'ecc_40k_{i}_{j}.npy',ecc)
        gc.collect()
    ...
    zmax = o.zmax()
    ecc = o.e()
    np.save(dire + f'zmax_40k_{i}_8.npy',zmax)
    np.save(dire + f'ecc_40k_{i}_8.npy',ecc)
    del o
    gc.collect()

    ecc = []
    for j in range(9):
        ecc.append(np.load(dire + f'ecc_40k_{i}_{j}.npy'))
    ecc = np.hstack(ecc)

    zmax = []
    for j in range(9):
        zmax.append(np.load(dire + f'zmax_40k_{i}_{j}.npy'))
    zmax = np.hstack(zmax)
    rg = np.load(dire+ f'rg_{i}.npy')
    erg = np.load(dire +f"erg_{i}.npy")
    data[['zmax','ecc','rg','erg']] = np.vstack((zmax,ecc,rg,erg)).T

    print(f'{i} th orbit calc. done!')
    thin_and_thick_grad(i,data,data_name='W23',rg_name='rg',feh_name='FEH_APOGEE',age_name='age',erg_name='erg',afe_name='AFE_APOGEE',zmax_name='zmax',efeh_name='err_feh_apogee')
