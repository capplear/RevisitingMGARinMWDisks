...
o = Orbit(re_xxvvs,**get_physical(MWPotential2014))
samp_rgs=o.rguiding(pot=MWPotential2014).reshape((N,100))
rg = np.mean(samp_rgs,axis=1)
erg = np.std(samp_rgs,axis=1)
np.save(r'/home/chenao/for_draft/rg.npy',rg)
np.save(r'/home/chenao/for_draft/erg.npy',erg)
for i in range(8):
    o=Orbit(normalized[40000*i:40000*(i+1),:5],**get_physical(MWPotential2014))
    ts=np.linspace(0.,150.,10001)
    o.integrate(ts,MWPotential2014)
    zmax = o.zmax()
    ecc = o.e()
    np.save(f'/home/chenao/for_draft/zmax_40k_{i}.npy',zmax)
    np.save(f'/home/chenao/for_draft/ecc_40k_{i}.npy',ecc)
    del o
    gc.collect()

o=Orbit(normalized[320000:,:5],**get_physical(MWPotential2014))
ts=np.linspace(0.,150.,10001)
o.integrate(ts,MWPotential2014)
zmax = o.zmax()
ecc = o.e()
np.save(r'/home/chenao/for_draft/zmax_40k_8.npy',zmax)
np.save(r'/home/chenao/for_draft/ecc_40k_8.npy',ecc)
del o
gc.collect()

ecc = []
for i in range(9):
    ecc.append(np.load(f'/home/chenao/for_draft/ecc_40k_{i}.npy'))
ecc = np.hstack(ecc)

zmax = []
for i in range(9):
    zmax.append(np.load(f'/home/chenao/for_draft/zmax_40k_{i}.npy'))
zmax = np.hstack(zmax)
rg = np.load('/home/chenao/for_draft/rg.npy')
erg = np.load("/home/chenao/for_draft/erg.npy")
...
def thin_and_thick_grad(data,i=0,data_name='W23', a=-0.3, b=0.12, k=-0.14,rg_name='rg',feh_name='FEH_APOGEE',age_name='age',erg_name='erg',afe_name='AFE_APOGEE',zmax_name='zmax',efeh_name='err_feh_apogee',ecc_name=False):
    if ecc_name:
    ...
    np.save(f"thin_meta_grad{i}.npy",thin_meta_grad)
    np.save(f"thick_meta_grad{i}.npy",thick_meta_grad)
    np.save(f"thin_err{i}.npy",thin_err)
    np.save(f"thick_err{i}.npy",thick_err)
    return 0
thin_and_thick_grad(data)